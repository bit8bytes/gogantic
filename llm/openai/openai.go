...
func (oc *OpenAiClient) GenerateContent(ctx context.Context, messages []llm.Message) (*llm.ContentResponse, error) {
	requestPayload := Chat{
...
func (oc *OpenAiClient) StreamContent(ctx context.Context, messages []llm.Message, streamHandler llm.StreamHandler) error {
...
func (oc *OpenAiClient) GenerateEmbedding(ctx context.Context, input string) (llm.EmbeddingResponse, error) {
	...
	if resp.StatusCode != http.StatusOK {
		bodyBytes, _ := io.ReadAll(resp.Body)
		return llm.EmbeddingResponse{}, errors.New("API error: " + string(bodyBytes))
	}
	...
	if len(embeddingResponse.Data) == 0 {
		return llm.EmbeddingResponse{}, errors.New("no embedding returned")
	}

	return llm.EmbeddingResponse{Embedding: embeddingResponse.Data[0].Embedding}, nil
}
